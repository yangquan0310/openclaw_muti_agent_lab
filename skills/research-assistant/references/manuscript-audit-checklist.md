...
> 触发：synthesize 输出完成 + 7-agent 评审 + originality check 通过后
...
| 35 | 引用格式 APA 7 / 期刊要求（参见 apa7-citation-checklist.md）| |
...
research-assistant audit --input manuscript.md \
                          --checklist references/manuscript-audit-checklist.md \
...
synthesize 输出
        ↓
references/synthesize-peer-review.md（7-agent 评审）
        ↓
references/originality-checklist.md（30 项原创性）
        ↓
references/apa7-citation-checklist.md（50 项 APA）
        ↓
references/manuscript-audit-checklist.md（60 项最终审计）← 本文
        ↓
所有 P0 = 0，P1 ≥ 80% 已修 → 投稿
```
