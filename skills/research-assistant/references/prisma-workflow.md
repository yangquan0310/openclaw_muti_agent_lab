...
> ⚠️ **本 SOP 是 agent 流程级，无对应 Python 实现**——9 阶段全部由 agent 手动跑 LLM prompts + 人工判断。本工具（research-assistant）只提供 search / download / manage 工具能力，PRISMA 流程质量靠 agent 本身。
...
| 7 流程图 | mermaid + 模板 |
| 8 综合 | `synthesize extract`（check/fix v5.16.0 未迁移；APA 核验走 `apa7-standards.md`）|
| 9 GRADE | 模板 + 人工评级 |
