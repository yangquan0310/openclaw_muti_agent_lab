...
class Manager:
    ...
    def export_notes(self, theme: str, output_dir: str = "knowledge/note") -> str:
        """
        将当前筛选结果导出为 Markdown 笔记（简洁格式，供快速浏览）
        参数:
            theme: 笔记主题名
        ...
        lines = [
            ...
            f"> 共 {len(self._current_papers)} 篇论文",
            "",
            "---",
            "",
        ]
        
        for i, paper in enumerate(self._current_papers, 1):
            ...
            if importance or ptype:
                lines.append(f"**标签**: {importance} {ptype}".strip())
            if url:
