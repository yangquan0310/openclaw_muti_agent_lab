...
class WikiZoteroManager:
    ...
    def find_missing_zotero_keys(self):
        """列出缺 zotero_item_key 的 source"""
        return [s for s in self.list_wiki_sources() if not s['has_zotero_item_key']]
    ...
    def check_drift(self):
        """检查所有 wiki source ↔ Zotero ↔ WebDAV 一致性

        返回：
        {
          'ok': [...],                  # 正常
          'missing_key': [...],         # 缺 zotero_item_key
    ...
    def generate_drift_report(self, output_path=None):
        ...
        lines = [f'# 漂移报告 - {date}', '',
                 f'> 自动生成 by WikiZoteroManager v5.15.0',
                 '',
                 f'## 🟢 OK ({len(drift["ok"])})', '']
        for s in drift['ok']:
