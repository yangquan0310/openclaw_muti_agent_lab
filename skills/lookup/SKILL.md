...
lookup index  -r ./skill/references                  # 默认输出到 <references>/../index
lookup index  -r ./skill/references -i /tmp/my-index  # 指定输出位置

# 搜索
lookup search -i <index或manifest.json路径> <关键词>

# 列出已索引文件
lookup list   -i <index或manifest.json路径>
```
...
- **路径兼容**：`--index` 参数接受目录路径或直接指向 `manifest.json` 文件

## 快速调用

```bash
lookup index  -r ~/.openclaw/workspace/mathematician/skills/mathematician/references
lookup search -i ~/.openclaw/workspace/mathematician/skills/mathematician/index 工作流
...
| 版本 | 日期 | 更新 |
|------|------|------|
| 1.0.0 | 2026-05-24 | 初始版本，改为指向文件/目录的参数化设计 |