...
SCRIPT_INDEX_END=$(grep -n "^---" "${TOOLS_FILE}" | grep -A1 -B1 "脚本索引" | tail -n1 | cut -d: -f1)
...
PROJECT_INDEX_END=$(grep -n "^---" "${TOOLS_FILE}" | grep -A1 -B1 "项目库" | tail -n1 | cut -d: -f1)
