...
class Calculate:
    ...
    def main(cls):
        ...
        if args.command == "basic":
            result = cls.basic_calc(args.a, args.b, args.op)
            print(json.dumps({"result": result}))
        elif args.command == "matrix":
            A = json.loads(args.A)
            B = json.loads(args.B) if args.B else None
            result = cls.matrix_ops(A, B, args.op)
            print(json.dumps(result, indent=2))
        elif args.command == "integrate":
            result = cls.numerical_integration(args.func, args.a, args.b, args.method)
            print(json.dumps(result, indent=2))
        elif args.command == "ode":
            y0 = [float(x) for x in args.y0.split(",")]
            t_span = [args.t0, args.t1]
            result = cls.solve_ode(args.func, y0, t_span)
            print(json.dumps(result, indent=2))
        elif args.command == "root":
            x0_str = args.x0.split(",")
            x0 = [float(x) for x in x0_str]
            result = cls.root_finding(args.func, x0, args.method)
            print(json.dumps(result, indent=2))
            ...
            result = cls.interpolation(x_data, y_data, x_eval, args.method)
            print(json.dumps(result, indent=2))
        else:
            parser.print_help()
...
if __name__ == "__main__":
    raise SystemExit(Calculate.main())
