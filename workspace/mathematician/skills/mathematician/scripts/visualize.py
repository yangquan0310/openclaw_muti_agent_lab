...
def main():
    ...
    if args.command == "function":
        result = plot_function(args.func, [args.x0, args.x1], args.output, args.title)
        print(json.dumps(result, indent=2))
    elif args.command == "data":
        x_data = [float(x) for x in args.x.split(",")]
        y_data = [float(y) for y in args.y.split(",")]
        result = plot_data(x_data, y_data, args.output, args.title, args.xlabel, args.ylabel)
        print(json.dumps(result, indent=2))
    elif args.command == "histogram":
        data = [float(d) for d in args.data.split(",")]
        result = plot_histogram(data, args.output, args.bins, args.title, args.xlabel, args.ylabel)
        print(json.dumps(result, indent=2))
    elif args.command == "scatter":
        x_data = [float(x) for x in args.x.split(",")]
        y_data = [float(y) for y in args.y.split(",")]
        result = plot_scatter(x_data, y_data, args.output, args.title, args.xlabel, args.ylabel)
        print(json.dumps(result, indent=2))
    elif args.command == "bar":
        categories = args.categories.split(",")
        values = [float(v) for v in args.values.split(",")]
        result = plot_bar(categories, values, args.output, args.title, args.xlabel, args.ylabel)
        print(json.dumps(result, indent=2))
    elif args.command == "box":
        data_groups = [[float(d) for d in group.split(",")] for group in args.data.split(";")]
        labels = args.labels.split(",")
        result = plot_box(data_groups, labels, args.output, args.title)
        print(json.dumps(result, indent=2))
    else:
        parser.print_help()
