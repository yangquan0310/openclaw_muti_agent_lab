...
| 临时文件 | ~/.openclaw/workspace/reviewer/temp/{任务}/ | 任务执行过程中的临时文件存储目录 |
| 实验室仓库 | ~/实验室仓库/ | 实验室仓库 |
| 实验室项目 | ~/实验室仓库/项目文件/ | 实验室各个项目 |
...
| 触发条件 | 脚本编号 | 脚本名称 | 功能描述 |
|----------|----------|----------|----------|
| 收到论文评审请求，需要评估论文质量 | S1 | 质量审查脚本 | 对论文进行多维度质量审查，生成结构化评审报告和修改建议 |
| 需要检查论文格式规范 | S2 | 格式规范脚本 | 对照学术标准检查文档格式，提供详细的格式修正建议 |
| 完成论文评审，需要系统性记录审稿意见 | S3 | 审稿意见记录脚本 | 结构化记录评审意见，建立问题追踪系统，支持版本控制 |
| 工作记忆维护 | - | 维护工作记忆.sh | 脚本文件: 维护工作记忆.sh |
