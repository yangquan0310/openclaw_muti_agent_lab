...
echo "## 任务1：TOOLS.md 更新" >> "$LOG_FILE"
echo "" >> "$LOG_FILE"

# 1. 更新TOOLS.md - 备份当前版本
if [ -f "$TOOLS_FILE" ]; then
    backup_file="$TOOLS_FILE.backup.$(date +%Y-%m-%d)"
    cp "$TOOLS_FILE" "$backup_file"
    echo "✅ 已备份 TOOLS.md 到 $backup_file" >> "$LOG_FILE"
    
    # 更新项目库列表（扫描项目文件夹）
    echo "📁 扫描项目文件夹..." >> "$LOG_FILE"
    project_dir="$HOME/实验室仓库/项目文件"
    if [ -d "$project_dir" ]; then
        project_count=$(ls -1 "$project_dir" 2>/dev/null | wc -l)
        echo "   发现 $project_count 个项目" >> "$LOG_FILE"
    fi
    echo "✅ TOOLS.md 更新检查完成" >> "$LOG_FILE"
else
...
# 定义标准文件夹列表
standard_dirs=("scripts" ".openclaw" "temp")
unexpected_dirs=()
