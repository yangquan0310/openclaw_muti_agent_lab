...
示例:
  ...
  python3 visualize.py surface --func "np.sin(np.sqrt(x**2+y**2))" --x-range "[-5,5]" --y-range "[-5,5]"
    ...
    if args.command == 'function':
        x1_val = eval(args.x1)
        Visualize.plot_function(args.func, args.x0, x1_val, args.title, args.xlabel, args.ylabel)

    elif args.command == 'phase':
        y0_list = json.loads(args.y0)
        t1_val = float(args.t1) if isinstance(args.t1, (int, float)) else eval(args.t1)
        Visualize.plot_phase_trajectory(args.func, y0_list, [args.t0, t1_val], args.title)

    elif args.command == 'field':
        x_range = json.loads(args.x_range)
        y_range = json.loads(args.y_range)
        Visualize.plot_field(args.potential, x_range, y_range, args.title)

    elif args.command == 'surface':
        x_range = json.loads(args.x_range)
        y_range = json.loads(args.y_range)
        Visualize.plot_3d_surface(args.func, x_range, y_range, args.title)
