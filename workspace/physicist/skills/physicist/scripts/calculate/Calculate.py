...
class Calculate:
    ...
    @staticmethod
    def integrate(func_str, a, b, method='quad'):
        """数值积分"""
        try:
    ...
    @staticmethod
    def ode(func_str, y0, t_span, method='RK45'):
        """求解常微分方程"""
        try:
...
示例:
  ...
  python3 calculate.py ode --func "[y[1], -y[0]]" --y0 "[0, 1]" --t0 0 --t1 10
    ...
    if args.command == 'basic':
        result = Calculate.basic(args.a, args.b, args.op)
        print(f"结果: {result}")

    elif args.command == 'matrix':
        A = json.loads(args.A)
        result = Calculate.matrix(A, args.op)
        print(f"结果: {result}")

    elif args.command == 'integrate':
        result = Calculate.integrate(args.func, args.a, args.b, args.method)
        if result:
        ...
        result = Calculate.ode(args.func, y0, [args.t0, args.t1], args.method)
        if result:
