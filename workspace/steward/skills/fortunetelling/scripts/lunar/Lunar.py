...
class Lunar:
    ...
    @classmethod
    def to_solar(cls, year: int, month: int, day: int) -> datetime:
        """
        将阴历日期转换为公历日期
        ...
        lunar_days = sum(cls.LUNAR_MONTHS[:month]) + day - 1
        offset = int(lunar_days * 29.53)
        solar = base_solar + timedelta(days=offset + 11)
        return solar

    @classmethod
    def get_ganzhi(cls, year: int, month: int = 1, day: int = 1) -> tuple:
        """
        计算干支纪年
        ...
        """
        # 计算年干支
        year_gan = cls.TIANGAN[(year - 4) % 10]
        year_zhi = cls.DIZHI[(year - 4) % 12]

        # 计算月干支
        month_gan = cls.TIANGAN[(year * 12 + month + 3) % 10]
        month_zhi = cls.DIZHI[(month + 1) % 12]

        # 计算日干支（简化）
        base_day = datetime(2000, 1, 1)
        base_ganzhi = (37, 0)  # 2000年1月1日为庚辰
        days_diff = (datetime(year, month, day) - base_day).days
        day_gan = cls.TIANGAN[(base_ganzhi[0] + days_diff) % 10]
        day_zhi = cls.DIZHI[(base_ganzhi[1] + days_diff) % 12]
    ...
    def main():
        ...
        solar = Lunar.to_solar(args.year, args.month, args.day)
        ganzhi = Lunar.get_ganzhi(args.year, args.month, args.day)
        ...
        print(f"月柱: {ganzhi[1]}")
        print(f"日柱: {ganzhi[2]}")
        print(f"时柱: {Lunar.get_hour_ganzhi(ganzhi[2][0], args.hour)}")
