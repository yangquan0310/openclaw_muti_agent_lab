---
name: "manager"
description: "大管家科研项目管理入口 v5.15.0：派发任务/协调子代理/项目/wiki/Quarto"
author: Yang Quan
metadata:
  openclaw:
    emoji: 📋
...
| 版本 | 日期 | 更新 |
|------|------|------|
| **5.15.1** | **2026-08-04** | **🆕 L3 派发卡不设 parents 硬链接（踩坑修正）**：实测 dashboard-workbench 项目——L3 卡通过 `parents` 挂 L2 卡会被 workboard 依赖机制自动降级回 todo（源码 `card dependencies are not done`），dispatch 选不到（`started=0`）。**修正**：L3 卡不设 parents（可设 `createdByCardId` 保留归属），L2 卡用 notes/comment 软关联追踪 L3。card-granularity.md v1.0.0 → v1.0.1。 |
