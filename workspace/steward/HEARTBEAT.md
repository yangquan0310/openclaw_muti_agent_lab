...
| 每日 08:00 | 大管家（steward） | `2cd2fbc1-8357-471a-a030-5b33a3f8d6fc` | 老板八字运势日报 | 每日早 8:00 给老板推送八字运势日报：排盘 + 9-23 时流时速览 + 五维运势（事业/人际/姻缘/健康/性格）+ 餐饮推荐（午晚餐）+ 具体行动表；流派「基于格局分析的传统命理现代化」；**输出格式：list，不用表格**（2026-08-01 老板拍板改）；**v2.10.0 结构**（2026-08-30 22:46 老板拍板）：第二节固定**三节结构，顺序不可颠倒**：**① 天干交互 → ② 地支交互 → ③ 天地交互**；分析对象 = **流年柱 + 流月柱 + 流日柱 与 命盘四柱的交互**（不只流日）；① 天干交互 = 流年/流月/流日天干 vs 命盘天干的合冲（五合/四冲）；② 地支交互 = 流年/流月/流日地支 vs 命盘地支的合冲刑破（六合/三合/六害/冲/刑/破）；③ 天地交互 = 地支透干 + 天干通根（每个参与天干查地支根气、关键地支藏干查是否透出，落到用神/忌神得力失势）；**每个交互条目（天干或地支）下面再分子 list**（「引动」一行 + 受影响的五维各一行，禁止一行塞满）；**每条合冲刑破必须过成立条件判定**（六合 5 条件 / 三合 4 条件 / 三会 3 条件，半合半会被冲即破，无引化即「合而不化」，严禁张口就成局——2026-08-23 今日日报四条合会局全部误判实锤），**五维影响直接并入每条交互条目**（💼事业/🤝人际/💕姻缘/💪健康/🎭性格，不单独开五维块），**流时只列重要影响**（不罗列十神细节），**只分析流日/流月对宫位与十神星（宫星）的影响，不分析对神煞的引动**（羊刃/飞刃/天喜/灾煞/桃花/红鸾/天乙贵人/太极等神煞一律不写——2026-08-25 老板拍板）；v2.9.0 为 2026-08-30 22:37 拍板的天干地支双线版；v2.8.0 为 2026-08-25 拍板的合冲刑破主线版（含判定 + 五维并入）；announce → **当前 QQ bot 单聊**（2026-08-01 老板拍板从飞书私聊迁移到当前 QQ bot 会话 `chat_id=qqbot:c2c:9F3209C188797074357241443CF8176F`）。**2026-08-01 异常**：QQ bot C2C 通道返回 `API Error [/v2/users/.../messages]: invalid request`（连"测试"都不通），按 OpenClaw 文档是「老板未激活 QQ bot 主动发消息通道」症状——需老板先用 QQ 给 bot 发任意消息激活 C2C。**临时回退策略**：本期 cron 暂时切回飞书私聊 `ou_25cf20a1973aecc51f73d8e2800d7f7e`（保证日报不卡死），等老板激活 QQ bot 后再切回。 | `{"enabled": "已启用，QQ C2C 暂时异常，已临时回退飞书"}` |
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| 1.18.0 | 2026-08-30 | **🆕 T043 日报格式 v2.9.0 → v2.10.0**（老板 2026-08-30 22:46 拍板「第二节就这么改，分析流年流月流日与命盘的天干交互，地支交互和天地交互对今日影响分析」）：① **三节结构，顺序固定**——第二节 = ① 天干交互 → ② 地支交互 → ③ 天地交互（老板问「三个的顺序是什么？」→ 我建议按字面顺序 → 老板拍板确认）；② **分析对象扩到流年+流月+流日 vs 命盘**（不只流日）；③ 天干交互 = 流年/流月/流日天干 vs 命盘天干的合冲（五合/四冲）；④ 地支交互 = 流年/流月/流日地支 vs 命盘地支的合冲刑破；⑤ 天地交互 = 地支透干 + 天干通根（独立成节）；⑥ 保留 v2.9.0 全部规则（子 list 格式/判定条件/五维并入/不分析神煞）。**操作**：cron prompt 已更新（updatedAtMs 1788101249518），HEARTBEAT T043 行同步 v2.10.0。 |
| 1.17.0 | 2026-08-30 | **🆕 T043 日报格式 v2.8.0 → v2.9.0**（老板 2026-08-30 22:37 拍板，批今日日报「这个第二节list格式不好看：1 每一个地支交互下面再分list；2 缺少了地支透干，天干通根的交互分析；3 天干之间的合冲也缺少」）：① **双线主线**——今日分析改为「天干交互 + 地支交互」双线：天干线 = 天干合冲（五合/四冲）+ 透干通根；地支线 = 六合/三合/六害/冲/刑/破；② **每个交互条目（天干或地支）下面再分子 list**——主条目 = 交互名 + 判定；子条目 = 「引动」一行 + 受影响的五维各一行（禁止一行塞满）；③ **天干合冲必写**（天干五合甲己/乙庚/丙辛/丁壬/戊癸 + 天干四冲甲庚/乙辛/丙壬/丁癸，有则分析落宫星落五维，无则一句带过）；④ **地支透干 + 天干通根必写**——天干通根（透干天干查地支同五行藏干根气，根深力实/无根虚浮）+ 地支透干（关键地支藏干查是否透出，透出显象有力/藏而不透暗动待机）→ 落到用神/忌神得力失势判断。**操作**：cron prompt 已更新（`cron edit 2cd2fbc1 --message`，updatedAtMs 1788100805406），HEARTBEAT T043 行同步 v2.9.0。保留 v2.8.0 全部判定规则（六合 5 条件/三合 4 条件/三会 3 条件/五维并入/不分析神煞）。 | **🆕 T043 日报格式 v2.7.0 → v2.8.0**（老板 2026-08-25 17:57 拍板，批今日日报「你的整个分析，分析流日对神煞的引动？我不想分析对神煞的引动，对宫星的影响即可！不需要分析对神煞的影响」）：① **只分析流日/流月对宫位与十神星（宫星）的影响**——各星 = 十神星（正财/偏财/正官/七杀/正印/偏印/食神/伤官/比肩/劫财），**不再写羊刃/飞刃/天喜/灾煞/桃花/红鸾/天乙贵人/太极等神煞**（星被合动 = 激活，星被冲破 = 受损）；② 三层落地从「各宫×各星×各身体部位」改为「各宫×各宫星（十神）×各身体部位」；③ 删除 `--shensha` 调用 + 命中神煞速查段 + 神煞激活判断原则；④ DO/DON'T 各加一条「不分析神煞引动」（2026-08-25 拍板）。**操作**：cron prompt 已更新（`cron edit 2cd2fbc1 --message`，updatedAtMs 1787651952420），HEARTBEAT T043 行同步 v2.8.0。 |
