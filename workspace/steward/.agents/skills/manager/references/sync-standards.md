...
|------|---------|-----------------|
| 大管家领取时 | 添加主任务行，备注 `card={{card_id}}` | `workboard_create({...})` + `workboard_comment({ id, body: "sessionKey=..." })` |
...
| 大管家核验 | 标记 🔄 → ✅ | `workboard_complete({ id, summary, proof: { status: "passed" } })` |
| 出错 | 标记 🔄 → ❌ | `workboard_block({ id, reason: "..." })` 或 `workboard_complete({ id, summary, proof: { status: "failed" } })` |
