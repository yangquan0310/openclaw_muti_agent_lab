...
> **v1.12.0 重大补充**（2026-06-06 老板拍板）：
> 1. **通知模板 4 要素适用场景扩充**——v1.11.0 只用于 session 派发，**v1.12.0 扩充**到 session 派发 + IM 派发
> 2. **IM 群派发 5 段模板 = 通知模板 4 要素的群派发实例**——task_desc = 任务标题，@代理 = 艾特，CARD_ID（workboard 信息）= CARD_ID，前置要求 = 操作步骤，完成反馈 = 反馈要求
> 3. **不修改其他 9 个模板**（保持 v1.11.0）
...
**通知模板 4 要素**（v1.11.0 老板拍板，v1.12.0 扩充适用场景）：

| 要素 | 含义 | session 派发示例 | **IM 派发示例**（v1.12.0 扩充）|
|------|------|------------------|------------------------------|
| **任务标题** | 告诉代理做什么 | 写一首词（蝶恋花/水调歌头自选）| task_desc 字段 |
| **CARD_ID** | 告知 workboard 卡 ID（让代理自管）| `34b4d37f-704b-4a8e-bdeb-d2db19b73a73` | workboard 信息 段（cardId/short + sessionKey + dashboard URL）|
| **操作步骤** | 按 v3.5.0 范式自管 workboard | claim → comment → proof → complete | 前置要求 段（步骤/路径/读什么文件/查看 .md）|
| **反馈要求** | 完成后怎么反馈 | 主 DM 回复 + 简短说明 | 完成反馈 段（在群里艾特大管家 + 汇报结果）|

> **通知模板 4 要素**——v1.12.0 老板拍板**适用**：
> - **session 派发**（sessions_spawn）—— task 用 4 要素结构
> - **IM 派发**（群里艾特）—— 5 段模板**就是** 4 要素的**群派发实例**
> 
> **结构清晰**——让代理**知道做什么 / 在哪做 / 怎么做 / 怎么反馈**。
