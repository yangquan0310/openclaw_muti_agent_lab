...
> **v3.8.0 重大补充**（2026-06-06 老板拍板）：
> 1. **IM 群派发 5 段模板 = 通知模板 4 要素的群派发实例**——task_desc = 任务标题，@代理 = 艾特，CARD_ID（workboard 信息）= CARD_ID，前置要求 = 操作步骤，完成反馈 = 反馈要求
> 2. **不修改其他模板**（保持 v3.7.0）

> **v3.7.0 重大补充**（2026-06-06 老板拍板）：
> 1. **任务四要素**（建卡 note 核心）——任务目标 / 任务约束 / 输入路径 / 输出路径
> 2. **通知模板 4 要素**（派发核心）——任务标题 / CARD_ID / 操作步骤 / 反馈要求
> 3. 其他模板保持（10 个原模板不变）
...
**详细模板内容**：见 workboard-guide.md v1.12.0 §三之5（10 个完整模板）。
...
**通知模板 4 要素**（v3.7.0 老板拍板，**v3.8.0 扩充适用场景**）——**派发通知核心内容**：

| 要素 | 含义 | **session 派发** | **IM 派发**（v3.8.0 扩充）|
|------|------|------------------|---------------------------|
| **任务标题** | 告诉代理做什么 | task 字段开头 | **task_desc** 段（5 段模板第 1 段）|
| **艾特** | 群里明确收件人 | （不需要）| **@代理** 段（5 段模板第 2 段）|
| **CARD_ID** | 告知 workboard 卡 ID | task 字段含 `**CARD_ID**` | **workboard 信息** 段（5 段模板第 3 段：cardId/short + sessionKey + dashboard URL）|
| **操作步骤** | 怎么干 | task 字段 step 列表 | **前置要求** 段（5 段模板第 4 段：步骤/路径/读什么文件）|
| **反馈要求** | 完成后怎么反馈 | task 字段末尾 | **完成反馈** 段（5 段模板第 5 段：在群里艾特大管家 + 汇报结果）|

**IM 派发 5 段模板**（task-flow-guide §二、群派发场景原有）**就是**通知模板 4 要素的群派发实例——v3.8.0 老板拍板扩充。

**派发双通道 + 通知模板**：
- **session 派发**（sessions_spawn）—— 4 要素结构（任务标题 / CARD_ID / 操作步骤 / 反馈要求）
- **IM 派发**（群里艾特）—— 5 段模板（任务标题 / @代理 / workboard 信息 / 前置要求 / 完成反馈）**= 4 要素 + @艾特**
