...
class Bazi:
    ...
    @classmethod
    def get_wuxing(cls, ganzhi: str) -> str:
        """获取干支的五行"""
        gan = ganzhi[0]
        zhi = ganzhi[1]
        return cls.WUXING.get(gan, '') + cls.DIZHI_WUXING.get(zhi, '')

    @classmethod
    def analyze(cls, year: int, month: int, day: int, hour: int,
        ...
        if is_lunar:
            lunar_obj = Lunar(year, month, day, hour, 0, 0)
        ...
        result['wuxing'] = {
            'year': cls.get_wuxing(result['year']),
            'month': cls.get_wuxing(result['month']),
            'day': cls.get_wuxing(result['day']),
            'hour': cls.get_wuxing(result['hour'])
        }

        result['rizhu'] = result['day'][0]
        result['rizhu_wuxing'] = cls.WUXING[result['day'][0]]
        result['gender'] = gender

        # 阳历和阴历信息
        result['solar'] = solar.toString()
        result['lunar'] = lunar_obj.toString()

        return result

    @classmethod
    def print_bazi(cls, bazi: dict):
        """打印八字排盘"""
        print("=" * 40)
        print("八字排盘")
        print("=" * 40)
    ...
    def main():
        ...
        bazi = Bazi.analyze(args.year, args.month, args.day, args.hour,
                            args.gender, args.lunar)
        Bazi.print_bazi(bazi)
