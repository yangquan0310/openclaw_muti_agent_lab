...
Synthesizer 类基于知识库生成文献综述和研究现状，协调 NoteExtractor 和 ReferenceChecker 完成笔记提取与引用检查。
...
加载模板：[文献综述模板.md](./文献综述模板.md) 和 [研究现状模板.md](./研究现状模板.md)
...
info = synthesizer.extract('/path/to/笔记.json')
...
#### 步骤 4：参照文章
- 参照模板：[文献综述模板.md](./文献综述模板.md) 和 [研究现状模板.md](./研究现状模板.md)
...
# 检查参考文献（方法调用时只传文档路径）
results = synthesizer.check_references("综述文档.md")
...
# 修复引用（可选）
fix_result = synthesizer.fix_references("综述文档.md", "修复后的文档.md")
```
...
综述合成的核心类，采用面向对象设计，协调 NoteExtractor 和 ReferenceChecker。
...
**参数:**
- `notes_path`: 知识库 JSON 文件路径

**返回:**
提取结果字典，包含文献数量、统计信息、研究问题列表、方法列表等。
...
> **注意**：知识库路径在初始化 `Synthesizer(kb1, kb2, ...)` 时绑定，不在 `check_references()` 方法中传入。
...
> **注意**：`fix_references` 需要先调用 `check_references` 构建引用映射。使用初始化时绑定的知识库，无需额外传入。
...
### 提取笔记信息
```bash
python3 Synthesizer.py extract --notes 笔记.json
```

### 检查参考文献（绑定一个知识库）
```bash
python3 Synthesizer.py check \
    --doc 综述文档.md \
    --kb 知识库1.json
```
...
| `check` | 检查引用子命令 |
| `fix` | 修复引用子命令 |
| `--notes` | 知识库JSON文件路径（extract时使用） |
| `--doc` | 待检查的Markdown文档路径（check/fix时使用） |
| `--kb` | 知识库JSON文件路径（可多次使用） |
| `--output` | 修复后输出路径（fix时使用） |
