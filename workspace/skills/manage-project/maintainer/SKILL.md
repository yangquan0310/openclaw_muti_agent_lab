...
### `archive_to_drafts(file_path, title=None, version=None, dry_run=False)`
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `file_path` | str | ✅ | 源文件路径（相对项目根目录或绝对路径） |
| `title` | str | ❌ | 指定标题（默认从文件名提取） |
| `version` | str | ❌ | 指定版本号（默认自动递增） |
| `dry_run` | bool | ❌ | 是否仅预览 |

**返回：** 归档后的相对路径，失败返回 `None`
...
旧版本/备份文件归档到 `临时数据/草稿/` 目录时，遵循以下规范：
...
**归档流程：**
1. 从文件名提取标题（去掉 `backup`、`备份`、`v数字` 等后缀）
2. 按标题创建子目录：`临时数据/草稿/{标题}/`
3. 确定版本号：
   - 如果文件名含版本号（如 `论文_v2.md`），使用该版本号
   - 否则自动递增（扫描子目录中现有版本，取最大值+1）
4. 命名格式：`{标题}_{version}.md`

**示例：**
| 原文件名 | 归档路径 |
|---------|---------|
| `论文_backup.md` | `临时数据/草稿/论文/论文_v1.md` |
| `论文_v2.md` | `临时数据/草稿/论文/论文_v2.md` |
| `综述_旧版本.md` | `临时数据/草稿/综述/综述_v1.md` |
