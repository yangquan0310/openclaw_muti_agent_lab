...
4. **会话与工具分配**
   - **所有任务都使用命名会话**（通过 `sessionTarget` 指定）
   - 命名会话格式：`session:{TYPE}:{任务标识}`
...
- 子任务：① … → ② … → ③ …
- 命名会话：
  - 复用现有：`session:XXX:xxx`（已存在，共享上下文）
  - 或新建：`session:XXX:xxx`（新创建）
- 任务看板预览：
- 成功标准：…
- 相关文件位置：…
- **计划已制定完成，请确认会话分配和是否可以开始执行？**
```
---
...
| `session_target` | string | ❌ | 命名会话标识，如 `session:PROJECT:xxx`。若省略则自动检查复用或创建 |
...
### 子任务清单
| 序号 | 子任务 | 命名会话 | 预计时间 | 依赖 | 状态 |
|------|--------|----------|----------|------|------|
| 1 | [描述] | [session:XXX:xxx] | [时长] | [依赖] | pending |
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.1.0 | 2026-04-26 | 统一使用命名会话，取消一次性任务区分；添加上下文共享检查步骤 |
