...
name: memory_table
description: >
  工作记忆记忆表管理。维护工作记忆的表格结构、数据完整性和定期清理。只追踪命名会话，一次性任务无需追踪。
version: 1.0.0
author: 大管家
dependencies: []
exports:
...
> 维护工作记忆的表格结构和数据完整性
> 
> **追踪范围**：只追踪命名会话（通过 `sessionTarget` 指定）。一次性任务（无命名会话）执行完即销毁，无需在工作记忆中追踪。
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.1.0 | 2026-04-26 | 统一使用命名会话，取消一次性任务区分；更新表格结构规范 |
