...
#!/usr/bin/env python3
"""
用tencent_plan总结数字化存储与自传体记忆知识库，并按7个主题分类
"""
...
print("="*80)
print("用tencent_plan总结并分类数字化存储与自传体记忆知识库")
print("="*80)
...
# 6. 初始化OpenAI client用于tencent_plan
import openai

api_key = os.environ.get(tencent_plan_config['api_key_env'])
if not api_key:
    print(f"错误: 请设置环境变量 {tencent_plan_config['api_key_env']}")
    sys.exit(1)
...
## 输出格式（必须严格遵守）
只返回一个数字：1、2、3、4、5、6或7，不要有其他内容。"""

# 8. 逐个分类（先分类，跳过总结以节省时间）
print("\n开始分类论文到7个主题（跳过总结以节省时间）...")
...
for i, paper in enumerate(papers, 1):
    ...
    try:
        ...
        topic_num_str = response.choices[0].message.content.strip()
...
print(f"\n保存完成: {KB_PATH}")
print("="*80)
print("\n注意：为节省时间，本次只做了分类，没有用LLM总结文献。")
print("如需总结，请运行Summarizer.py。")
print("="*80)
