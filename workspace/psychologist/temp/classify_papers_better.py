...
# 使用标准tencent而不是tencent_plan
tencent_config = config['llm']['providers']['tencent']

print("="*80)
print("用tencent标准模型分类数字化存储与自传体记忆知识库")
print("="*80)
...
api_key = os.environ.get(tencent_config['api_key_env'])
if not api_key:
    print(f"错误: 请设置环境变量 {tencent_config['api_key_env']}")
    sys.exit(1)
