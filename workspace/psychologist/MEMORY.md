...
| **T-CFPPS v1 越界教训（2026-06-06 22:02 老板驳回）** | **作为心理学家在 v1 综述中擅自将"影响因素"扩展为"链式整合模型 + 双轨结构假说 + 自我暴露中介桥梁"——这是违反"不擅自添加/修改任务范围"原则（SOUL.md + AGENTS.md）的典型越界。教训：(1) 任务范围严格遵守 owner 指定的科学问题方向，不自行升级为"理论贡献"或"模型构建"；(2) 收到任务时若默认"X→Y 列举"是低水平、需"提升到中介模型"——这是错位判断，owner 明确要 X→Y 形式就严格 X→Y；(3) 违反禁用词清单时 1 字都是驳回——v2 严格 grep 自检并保留可证据；(4) 老板写的"3 意群引言"是仅针对初稿引言，不是综述方向。已记 1 次。**v2 已重写交付（4500 字 X→Y 形式，6 禁用词正文 0 命中）**。 |
| **workboard 任务执行前** | **必须先调用 workboard_read 扫读卡片 comment 区和 events 列表，识别是否有 process correction / 撤回指令 / blocked 状态** |
