...
>
> **v3.3.0 重大变更**：统一 task JSON 取代分散状态键（plan/deviation/attribution/event/session_list/tools 合并为单个 `task:{runId}`）；移除 Cron 和 Diary 系统；人格更新改为 agent_end 触发；扩展为 6 维度（新增程序性记忆）。
...
|------|------|------------|----------|-------------|
| `before_prompt_build` | 复杂任务 + Plan 不存在/draft | `planning` | Plan | State.saveTask() |
| `before_prompt_build` | Plan pending_approval / revising | `planning` | Plan | State.saveTask() |
| `before_prompt_build` | Plan active | 执行上下文（当前阶段提醒） | Plan | State.getTask() |
| `llm_output` | Plan active | `monitoring` | Deviation | task.event.deviations.push() |
| `before_tool_call` | 会话工具调用 | —（仅记录） | Session | State.saveSession() |
| `after_tool_call` | 任意 | —（仅记录工具结果） | Tool | task.tools.push() |
| `agent_end` | 任意 | `working_memory` + `development` | Task | task.event → Memory.eventlog |
