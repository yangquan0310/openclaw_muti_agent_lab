...
>
> **v3.0.0 重大变更**：深度集成 OpenClaw 核心系统（Flow、Memory、State），从自研 JSON 文件迁移到核心 SQLite 持久化，实现跨会话恢复、并发安全和系统级状态管理。
...
└─────────────────────────────────────────┘
  ↑ Hook 事件触发         ↓ API 调用
  ↑ before_prompt_build   ↓ Flow.create
...
| 核心系统 | API 路径 | 插件层调用 | 底层存储 | 用途 |
|----------|---------|-----------|----------|------|
| **Task** | `api.runtime.tasks` | TaskAdapter | `tasks/runs.sqlite` | `task_runs`, `task_delivery_state` | 任务创建、查询、管理 |
| **Flow** | `api.runtime.flow` | FlowAdapter | `flows/registry.sqlite` | `flow_runs` | 应用Flow、计划工作流、阶段推进、等待用户确认 |
| **State** | `api.runtime.state` | StateAdapter | `state/agent-self-development/` (插件专属JSON文件) | `sticker-cache.json` 等 | 原子存储、事务、Plan/Session/Deviation/Attribution 状态 |
| **Memory** | `api.runtime.memory` | MemoryAdapter | `memory/{agentId}.sqlite` | `chunks`, `files`, `meta` | 归档、事件记录、历史查询 |
| **Log** | `api.runtime.log` | LogAdapter | `logs/commands.log`, `logs/config-audit.jsonl`, `logs/config-health.json` | 文本/JSONL文件 | 日志写入、读取、查询 |
...
| **Plan 存储** | 自研 JSON 文件 (`state/`) | 核心 Flow (`flows/`) | 持久化、跨会话恢复 |
| **Session 追踪** | 内存 + JSON | 核心 State API (`state/agent-self-development/`) | 并发安全、事务支持 |
| **归档系统** | 自研 `memory_table:{日期}` | 核心 Memory API (`memory/`) | 统一记忆管理、跨插件共享 |
| **状态管理** | `PluginState` 类（文件锁） | 核心 State 事务 | 原子操作、版本控制 |
| **应用Flow** | 自研 Plan 状态机 | 核心 Flow | 标准持久化、断点续传 |
...
| 组件 | 职责 | 依赖核心系统 | 底层存储 |
|------|------|-------------|----------|
| `StateAdapter` | 状态存储适配器 | `api.runtime.state` | `state/agent-self-development/` (插件专属JSON文件) |
| `TaskAdapter` | 任务适配器 | `api.runtime.tasks` | `tasks/runs.sqlite` |
| `FlowAdapter` | 应用Flow适配器 | `api.runtime.flow` | `flows/registry.sqlite` |
| `MemoryAdapter` | 记忆存储适配器 | `api.runtime.memory` | `memory/{agentId}.sqlite` |
| `LogAdapter` | 日志适配器 | `api.runtime.log` | `logs/commands.log`, `logs/config-audit.jsonl`, `logs/config-health.json` |
...
| `ArchiveManager` | 归档业务逻辑 | Memory | `memory/{agentId}.sqlite` |
...
| 适配器 | 核心方法 | 职责 | 底层存储 |
|--------|---------|------|----------|
| **StateAdapter** | `transaction()`, `savePlan()`, `getPlan()`, `saveSession()`, `getSession()`, `saveDeviation()`, `getDeviation()`, `saveAttribution()`, `getAttribution()` | 原子操作、Plan/Session/Deviation/Attribution 存储 | `state/agent-self-development/` (插件专属JSON文件) |
| **TaskAdapter** | `createTask()`, `getTask()`, `updateTask()`, `deleteTask()` | 任务创建、查询、更新、删除 | `tasks/runs.sqlite` |
| **FlowAdapter** | `createPlanFlow()`, `advancePhase()`, `waitForApproval()`, `getByRunId()` | 应用Flow、计划工作流、阶段推进、等待确认 | `flows/registry.sqlite` |
| **MemoryAdapter** | `archiveSession()`, `logEvent()`, `queryHistory()` | 归档、事件记录、历史查询 | `memory/{agentId}.sqlite` |
| **LogAdapter** | `write()`, `read()`, `query()` | 日志写入、读取、查询 | `logs/commands.log`, `logs/config-audit.jsonl`, `logs/config-health.json` |
...
|------|------|------------|----------|-------------|
| `before_prompt_build` | 复杂任务 + Plan 不存在/draft | `planning` | Plan | Flow.createManaged() |
| `before_prompt_build` | Plan pending_approval | `planning` | Plan | Flow.setWaiting() |
| `before_prompt_build` | Plan active | 执行上下文（当前阶段提醒） | Plan | Flow.get() |
...
| `TaskAdapter` | 核心 Task API 适配 | `createTask()`, `getTask()`, `updateTask()`, `deleteTask()` |
| `FlowAdapter` | 核心 Flow API 适配 | `createPlanFlow()`, `advancePhase()`, `waitForApproval()`, `getByRunId()` |
...
| `deviation:{runId}:{phaseId}` | Deviation | runId | 当前运行的偏差记录（State API） |
| `attribution:{runId}:{deviationId}` | Attribution | runId | 当前运行的归因记录（State API） |
| `session:{TYPE}:{任务族}` | Session | 长期 | 任务族会话（State API） |
| `event:{eventId}` | Event | 临时 | 单次任务执行事件（State API）→ agent_end 聚合到 EventLog |
| `diary:{YYYY-MM-DD}` | Diary | 日期 | 每日发展日记（Memory API） |
...
| `taskflow:{flowId}` | Flow | 长期 | 核心 Flow 实例（Flow API） |
    ...
    "entries": {
      "agent-self-development": {
        ...
        "config": {
          ...
          "integration": { "useCoreFlow": true, "useCoreMemory": true, "useCoreState": true }
        }
      }
    }
  }
...
| `integration.useCoreFlow` | boolean | `true` | 使用核心 Flow（v3.0.0 新增） |
