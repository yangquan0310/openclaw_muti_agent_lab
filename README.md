...
|----------|-----------|------|-----------|------|
| **Task** | `.openclaw/tasks/agent-self-development/tasks.db` | SQLite | TaskAdapter | 任务创建、查询、管理 |
| **Flow** | `.openclaw/flows/agent-self-development/flows.db` | SQLite | FlowAdapter | 计划工作流、阶段推进、等待用户确认 |
| **State** | `.openclaw/state/agent-self-development/` | JSON | StateAdapter | Plan/Session/Deviation/Attribution 状态 |
| **Memory** | `.openclaw/memory/agent-self-development/memory.db` | SQLite | MemoryAdapter | 归档、事件记录、历史查询 |
...
| **归档系统** | 自研 `memory_table:{日期}` | SQLite 数据库 (`memory/agent-self-development/memory.db`) | 统一目录结构、高效查询 |
| **状态管理** | `PluginState` 类（文件锁） | `StateAdapter`（文件系统 + 兼容接口） | 原子操作（单进程）、兼容未来核心 API |
| **应用Flow** | 自研 Plan 状态机 | `FlowAdapter`（SQLite + 兼容接口） | 标准化 Flow 数据结构 |
| **日志系统** | 控制台输出 | `LogAdapter`（按代理分文件 `{agentId}.log`） | 结构化日志、查询、审计 |
| **任务系统** | 未使用 | `TaskAdapter`（SQLite + 兼容接口） | 标准化任务管理 |
