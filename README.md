...
│  · 注入无可争议的流程/参数 skill           │
│                                          │
│  before_prompt_build → 注入 assessment / planning │
...
|------|------|------------|----------|-------------|
| `before_prompt_build` | 无 task | `assessment` | — | 不创建 task |
| `before_prompt_build` | task=draft | `planning` | Plan | State.saveTask() |
...
```bash
# 1. 安装插件
openclaw plugins install https://github.com/yangquan0310/openclaw_muti_agent_lab/releases/download/v3.4.0/agent-self-development-3.4.0.tgz --force
