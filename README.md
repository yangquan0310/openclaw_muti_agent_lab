...
>
> **v3.1.3 重大变更**：重构底层存储架构，Task/Flow/Memory 改为 SQLite 数据库，Log 改为按代理分文件，适配器层保留核心 API 兼容接口。
...
└─────────────────────────────────────────┘
  ↓ API 调用              ↑ Hook 事件触发
  ↓ State.savePlan        ↑ before_prompt_build
  ↓ State.saveDeviation   ↑ llm_output
...
|--------|---------|------|----------|--------|
| **StateAdapter** | `savePlan()`, `getPlan()`, `saveSession()`, `getSession()`, `saveDeviation()`, `getDeviation()`, `saveAttribution()`, `getAttribution()`, `saveEvent()`, `getEvent()` | Plan/Session/Deviation/Attribution 存储 | `state/agent-self-development/` (JSON文件) | 接口兼容核心 State API |
...
基于皮亚杰同化/顺应理论，每日 cron 触发时驱动 Agent 回顾昨日事件、撰写发展日记、更新人格文件（SOUL.md / IDENTITY.md / skills/README.md）。

| 对象 | 职责 | 生命周期 | 持久化 |
|------|------|--------|--------|
| **Event** | regulation 阶段撰写的事件记录（偏差/归因/调节措施/结果），供 development 每日回顾 | 单次事件，agent_end 时聚合到 EventLog | `state:event:{eventId}`（临时）→ 追加到 `memory:eventlog:{date}` |
...
核心 `memory:eventlog:{date}`（regulation 阶段撰写的事件）──→ development（每日回顾）
```
...
| `agent_end` | 任意 | EventManager（跨模块） | Event | State.events → Memory.eventlog |
| `before_prompt_build` | cron 消息 | `personality` | Diary | EventManager.queryEventLog() |
...
│   ├── personality/              # 人格模块
│   │   ├── module.js             # PersonalityModule（Diary 管理 / development skill 注入）
│   │   ├── diary-manager.js      # DiaryManager（日记业务逻辑）
│   │   └── SKILL.md              # Personality skill（同化/顺应指导）
│   ├── common/                   # 公共组件
...
| `StateAdapter` | 状态存储适配器（文件系统 + 兼容接口） | `savePlan()`, `getPlan()`, `saveSession()`, `getSession()`, `saveDeviation()`, `getDeviation()`, `saveAttribution()`, `getAttribution()`, `saveEvent()`, `getEvent()` |
...
| `CronAdapter` | Cron 任务适配器（读写 jobs.json） | `register()`, `unregister()`, `list()`, `get()` |
...
| `DiaryManager` | 日记业务逻辑 | `createDiary()`, `getDiary()`, `extractSignals()` |
